...
//			});
			...
			let gids = arrObj.map(function(item) {
				let uidCart = item.gid;
				let count = item.count;
				let zongjia = 0;
				//console.log(uidCart);
				return packageAjax_2({
					type: "post",
					...
					success: function(str) {
						...
						cartMain.onclick = function(ev){
							if(ev.target.className == 'reduce reSty'){
								let countCart = $('.sum').val().trim();
								let gidCart = ev.target.getAttribute('goods1-id');
...
//										$('.sum').val(res1);
									...
									let countCart = $('.sum').val().trim();
									let gidCart = ev.target.getAttribute('goods2-id');
									//console.log(ev.target);
									//console.log(countCart,gidCart,cookie_getdata('userid'));
									packageAjax_2({
										type : 'post',
										url : '../api/cartCount.php',
										data : 'countCart=' + countCart + '&uidCart=' + cookie_getdata('userid') + '&gidCart=' + gidCart,
